...
if ! (( "$OSTYPE" == "gnu-linux" )); then
  echo "docker-compose-wordpress-plugin-dev runs only on GNU/Linux operating system. Exiting..."
  exit
fi
...
###############################################################################
# 2.) Generate very basic plugin if it doesn't exist
###############################################################################
  ...
  if [[ ! -f $PROJECT_NAME/$PROJECT_NAME.php ]]; then
    touch $PROJECT_NAME/$PROJECT_NAME.php
    cat <<EOF> $PROJECT_NAME/$PROJECT_NAME.php
<?php
/*
  ...
  if [[ ! -f docker-compose.yml ]]; then
    ...
    services:
      ...
      wordpress:
        ...
        volumes:
          - ./$PROJECT_NAME:/var/www/html/wp-content/plugins/$PROJECT_NAME
        links:
          - database
        ports:
          - 80:80
    ...
    cat <<EOF> composer.json
{
    ...
    "description": "docker-compose-wordpress-plugin-dev project",
    "version": "1.0.0",
